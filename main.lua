...
HD_TILENAME = {
	...
	["D"] = {
		--#TOTEST: Also used in tutorial level 3 placement {3, 4}
...
-- -- Why is this not spawning the bottom room??
-- level_generation_method_structure_vertical(
-- 	{
-- 		subchunk_id = HD_SUBCHUNKID.MOTHERSHIPENTRANCE_TOP,
-- 		roomcodes = HD_ROOMOBJECT.FEELINGS["MOTHERSHIPENTRANCE"].genMethod.rooms[HD_SUBCHUNKID.MOTHERSHIPENTRANCE_TOP]
-- 	},
-- 	{
-- 		bottom = {
-- 			subchunk_id = HD_SUBCHUNKID.MOTHERSHIPENTRANCE_BOTTOM,
-- 			roomcodes = HD_ROOMOBJECT.FEELINGS["MOTHERSHIPENTRANCE"].genMethod.rooms[HD_SUBCHUNKID.MOTHERSHIPENTRANCE_BOTTOM]
-- 		}
-- 	},
-- 	{1, 4},
-- 	0
	...
	level_generation_method_structure_vertical(
		{
			...
			roomcodes = HD_ROOMOBJECT.FEELINGS["SNAKEPIT"].genMethod.rooms[HD_SUBCHUNKID.SNAKEPIT_TOP]
		},
		{
			middle = {
				subchunk_id = HD_SUBCHUNKID.SNAKEPIT_MIDSECTION,
				roomcodes = HD_ROOMOBJECT.FEELINGS["SNAKEPIT"].genMethod.rooms[HD_SUBCHUNKID.SNAKEPIT_MIDSECTION]
			},
			bottom = {
				subchunk_id = HD_SUBCHUNKID.SNAKEPIT_BOTTOM,
				roomcodes = HD_ROOMOBJECT.FEELINGS["SNAKEPIT"].genMethod.rooms[HD_SUBCHUNKID.SNAKEPIT_BOTTOM]
			}
		},
		{1, 2, 3, 4},
		1
...
HD_ROOMOBJECT.FEELINGS["TIKIVILLAGE"].genMethod.method = function()
	levelw, levelh = #global_levelassembly.modification.levelrooms[1], #global_levelassembly.modification.levelrooms
	...
	for room_y = levelh_start, levelh_end, 1 do
		for room_x = 1, levelw, 1 do
			...
			if path_to_replace_with ~= -1 then
				levelcode_inject_roomcode(path_to_replace_with, HD_ROOMOBJECT.FEELINGS["TIKIVILLAGE"].genMethod.rooms[path_to_replace_with], room_y, room_x)
			end
		end
	end
...
HD_ROOMOBJECT.FEELINGS["MOTHERSHIPENTRANCE"].genMethod.method = function()
	-- levelw, levelh = #global_levelassembly.modification.levelrooms[1], #global_levelassembly.modification.levelrooms
	-- structx = (math.random(2) == 2) and 1 or #global_levelassembly.modification.levelrooms

	-- levelcode_inject_roomcode(HD_SUBCHUNKID.MOTHERSHIPENTRANCE_TOP, HD_ROOMOBJECT.FEELINGS["MOTHERSHIPENTRANCE"].genMethod.rooms[HD_SUBCHUNKID.MOTHERSHIPENTRANCE_TOP], 1, structx)

	-- levelcode_inject_roomcode(HD_SUBCHUNKID.MOTHERSHIPENTRANCE_BOTTOM, HD_ROOMOBJECT.FEELINGS["MOTHERSHIPENTRANCE"].genMethod.rooms[HD_SUBCHUNKID.MOTHERSHIPENTRANCE_BOTTOM], 2, structx)
	...
	level_generation_method_structure_vertical(
		{
			...
			roomcodes = HD_ROOMOBJECT.FEELINGS["SACRIFICIALPIT"].genMethod.rooms[HD_SUBCHUNKID.SACRIFICIALPIT_TOP]
		},
		{
			middle = {
				subchunk_id = HD_SUBCHUNKID.SACRIFICIALPIT_MIDSECTION,
				roomcodes = HD_ROOMOBJECT.FEELINGS["SACRIFICIALPIT"].genMethod.rooms[HD_SUBCHUNKID.SACRIFICIALPIT_MIDSECTION]
			},
			bottom = {
				subchunk_id = HD_SUBCHUNKID.SACRIFICIALPIT_BOTTOM,
				roomcodes = HD_ROOMOBJECT.FEELINGS["SACRIFICIALPIT"].genMethod.rooms[HD_SUBCHUNKID.SACRIFICIALPIT_BOTTOM]
			}
		},
		{1, 2, 3, 4},
		2
...
-- 		prePath = false,
-- 		rooms = {
-- 			-- # TODO: replace tilecodes:
-- 				-- Hell Terrain ("h") with door ("9")
-- 				-- Mines Terrain/Temple Terrain/Pushblock ("r" (or "("))) with tilecodes seen in hd in-game
-- 		}
-- 	}
-- }
HD_ROOMOBJECT.FEELINGS["HELL"] = {
	genMethod = {
		prePath = true,
		rooms = {
...
HD_ROOMOBJECT.FEELINGS["HELL"].genMethod = function()
	level_generation_method_structure_vertical(
		{
			subchunk_id = HD_SUBCHUNKID.VLAD_TOP,
			roomcodes = HD_ROOMOBJECT.FEELINGS["HELL"].genMethod.rooms[HD_SUBCHUNKID.VLAD_TOP]
		},
		{
			middle = {
				subchunk_id = HD_SUBCHUNKID.VLAD_MIDSECTION,
				roomcodes = HD_ROOMOBJECT.FEELINGS["HELL"].genMethod.rooms[HD_SUBCHUNKID.VLAD_MIDSECTION]
			},
			bottom = {
				subchunk_id = HD_SUBCHUNKID.VLAD_BOTTOM,
				roomcodes = HD_ROOMOBJECT.FEELINGS["HELL"].genMethod.rooms[HD_SUBCHUNKID.VLAD_BOTTOM]
			}
		},
		{1, 2, 3, 4},
		2
...
HD_ROOMOBJECT.WORLDS[THEME.DWELLING] = {
	...
	idolRooms = {
		{"2200000022000000000000000000000000000000000000000000000000000000I000001111A01111"}
	},
	coffin_unlockable = {
		{
...
HD_ROOMOBJECT.WORLDS[THEME.JUNGLE] = {
	...
	idolRooms = {
		{"01000000100000I0000001BBBBBB10010000001011wwwwww1111wwwwww11113wwww3111111111111"}
	},
	-- coffin_unlockable = {
	-- 	{
	-- 		subchunk_id = HD_SUBCHUNKID.COFFIN_UNLOCKABLE,
...
function levelcode_inject(_chunkPool, _c_dim_h, _c_dim_w, _c_y, _c_x)
	chunkPool_rand_index = math.random(1, #_chunkPool)
	chunkCodeOrientation_index = math.random(1, #_chunkPool[chunkPool_rand_index])
...
function gen_levelrooms_nonpath(unlock, prePath)
	...
	if ((HD_WORLDSTATE_STATE == HD_WORLDSTATE_STATUS.TESTING) and (HD_ROOMOBJECT.TESTING[state.level].setRooms ~= nil)) then
		path_gen_setrooms(HD_ROOMOBJECT.TESTING[state.level].setRooms, prePath)
	end

	-- tutorial setrooms
	if ((HD_WORLDSTATE_STATE == HD_WORLDSTATE_STATUS.TUTORIAL) and (HD_ROOMOBJECT.TUTORIAL[state.level].setRooms ~= nil)) then
		path_gen_setrooms(HD_ROOMOBJECT.TUTORIAL[state.level].setRooms, prePath)
	end
	...
	for feeling, feelingContent in pairs(HD_ROOMOBJECT.FEELINGS) do
		if feeling_check(feeling) == true then
			if feelingContent.genMethod ~= nil then
				if (feelingContent.genMethod.prePath == nil and prePath == false) or (feelingContent.genMethod.prePath ~= nil and feelingContent.genMethod.prePath == prePath) then
...
function levelcreation_setlevelcode_path(num, wi, hi)
	...
	if roomPool ~= nil and roomPool[num] ~= nil then
		chunkPool_rand_index = math.random(1, #roomPool[num])
...
function gen_levelrooms_path()
	...
	(HD_ROOMOBJECT.WORLDS[state.theme] ~= nil and HD_ROOMOBJECT.WORLDS[state.theme].pathRooms == nil) then
		...
		while assigned_exit == false do
			...
			if num == HD_SUBCHUNKID.PATH and dropping == true then
				num = HD_SUBCHUNKID.PATH_NOTOP
				dropping = false
			end
			if num == HD_SUBCHUNKID.PATH_DROP then
				ind_off_y = 1
				if dropping == true then
					num = HD_SUBCHUNKID.PATH_DROP_NOTOP
				end
				dropping = true
			end
			if assigned_entrance == false then
				if num == HD_SUBCHUNKID.PATH_DROP then
					num = HD_SUBCHUNKID.ENTRANCE_DROP
					if reverse_path == true then
						num = HD_SUBCHUNKID.EXIT_NOTOP
					end
				else
					num = HD_SUBCHUNKID.ENTRANCE
					if reverse_path == true then
						num = HD_SUBCHUNKID.EXIT
					end
				end
			...
			elseif hi == levelh then
				...
				if assigned_exit == true then
					if num == HD_SUBCHUNKID.PATH_NOTOP then
						num = HD_SUBCHUNKID.EXIT_NOTOP
						if reverse_path == true then
							num = HD_SUBCHUNKID.ENTRANCE_DROP
						end
